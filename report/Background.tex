...
specifically, a GPU consists of some number of \textit{streaming
multiprocessors (SMPs)}. SMPs are general purpose, but have significant
...
some function. An SMP is capable execute a huge number of threads at once while being
able to efficiently switch context. We say that on an SMP, there can fit one workgroup,
so if an SMP can execute 128 threads, that SMP (and therefore that GPU) has a
maximum workgroup size of 128. Now this workgroup is an abstraction that we
use, since the threads on one SMP have fast communication with each other
through what we call \textit{shared memory}, this allows for synchronization of
the threads within the block.
